...
def train(train_data, valid_data, args, result_file):
    ...
    for epoch in tqdm(range(args.epoch), desc='handler.py > epoch'):
        ...
        print('| end of epoch {:3d} | time: {:5.2f}s | train_total_loss {:5.5f}'.format(epoch, (
        ...
        if (epoch + 1) % args.validate_freq == 0:
            ...
            performance_metrics = \
                validate(model, valid_loader, args.device, args.norm_method, normalize_statistic,
                         node_count, args.window_size, args.horizon,
...
def test(test_data, args, result_train_file, result_test_file):
    ...
    print('Performance on test set: MAPE: {:5.5f} | MAE: {:5.5f} | RMSE: {:5.5f}'.format(mape, mae, rmse), flush=True)
