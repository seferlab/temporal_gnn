...
def main(input_path,
    ...
    dfs = sorted(
        [StockDF(remove_csv_extension(path), pd.read_csv(f'{input_path}-{frequency}/{path}')) for path in paths],
        key=lambda stock_df: len(stock_df.df))[-num_assets:]
    merged_df = merge_by_bfilling(
        dfs, b_fill_days) if b_fill_days else merge_by_inner_join(dfs)
...
def merge_by_bfilling(dfs, b_fill_days):
    ...
    dfs_close = [stock_df.df[['Date', 'Close']].rename(columns={'Close': stock_df.name}).set_index('Date')
                 for stock_df in dfs_aligned]
