...
def main(input_path):
    ...
    last_days_of_weeks_df = \
        pd.DataFrame(datetimes, columns=['year', 'week_of_year', 'day_of_week']).groupby(['year', 'week_of_year'])[
            'day_of_week'].max()
    last_one_years_weeks_df = pd.DataFrame(last_days_of_weeks_df.reset_index().apply(
        lambda row: datetime.strptime(f"{row['year']} {row['week_of_year']} {row['day_of_week']}", '%G %V %u'), axis=1),
        columns=['Date'])
